...
def runCart(cart, numGames=1, render=False):
    ...
    for each_game in range(numGames):
        ...
        while True:
            ...
            if ((info["health"] > info["enemyHealth"]) and (info["enemyHealth"] <= 2)):
                # print(info)
...
def saveBestfromGenModel(cart, gen):
    cart.brain.save_weights("model5Gen{}.h5".format(gen))

def overwriteAllGenModels(carts):
    for i in range(len(carts)):
        carts[i].brain.save_weights("model5Cart{}.h5".format(i))
...
def run(carts, numGamesPerCart, numGenerations, childsPerSurvivor):
    ...
    while True:
        print("Reproducing...")
        childs = reproduction(bestChilds, childsPerSurvivor, 0.03, 0.015)
        ...
        genCounter += 1
        saveBestfromGenModel(bestChild, genCounter)
...
# Model 5 NN mais complexo 140, score de ataque * 1.5, crossover de 20000, numGamesPerCart = 20, selecting 7, saving fixed
